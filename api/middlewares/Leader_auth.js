...
const Leader_auth = async (req, res, next) => {
  ...
  try {
    ...
    if (
      !user ||
      (user.role.role !== "leader" && user.role.role !== "leadercumcreator")
    ) {
      return res
        .status(403)
        .json({ error: "You do not have leader privileges" });
    }
    if (user.status !== 1) {
